...
class LeoElementsProductsModel extends ObjectModel
{
    ...
    public function toggleStatusMT($field)
    {
        ...
        $result = Db::getInstance()->getRow('SELECT '.pSQL($field).' from  `'._DB_PREFIX_.'leoelements_products_shop` '.$where);
        $value = $result[$field]==1?0:1;

        if ($value == 1) {
            Db::getInstance()->execute('UPDATE `'._DB_PREFIX_.'leoelements_products_shop` set '.pSQL($field).' = "0" WHERE id_shop="'.$id_shop.'"');
            Db::getInstance()->execute('UPDATE `'._DB_PREFIX_.'leoelements_products` set '.pSQL($field).' = "0"');
        }
        
        Db::getInstance()->execute('UPDATE `'._DB_PREFIX_.'leoelements_products_shop` set '.pSQL($field).' = "'.$value.'" '.$where);
        Db::getInstance()->execute('UPDATE `'._DB_PREFIX_.'leoelements_products` set '.pSQL($field).' = "'.$value.'" '.$where1);
